...
constexpr std::uint_fast8_t MASK_moveLeft{ 0b0000'0001 }; // represents bit 0
constexpr std::uint_fast8_t MASK_moveRight{ 0b0000'0010 }; // represents bit 1
constexpr std::uint_fast8_t MASK_moveUp{ 0b0000'0100 }; // represents bit 2
constexpr std::uint_fast8_t MASK_moveDown{ 0b0000'1000 }; // represents bit 3
constexpr std::uint_fast8_t MASK_moveForward{ 0b0001'0000 }; // represents bit 4
constexpr std::uint_fast8_t MASK_moveBack{ 0b0010'0000 }; // represents bit 5

constexpr std::uint_fast8_t MASK_rotLeft{ 0b0000'0001 }; // represents bit 0
constexpr std::uint_fast8_t MASK_rotRight{ 0b0000'0010 }; // represents bit 1
constexpr std::uint_fast8_t MASK_rotUp{ 0b0000'0100 }; // represents bit 2
constexpr std::uint_fast8_t MASK_rotDown{ 0b0000'1000 }; // represents bit 3
constexpr std::uint_fast8_t MASK_rotTiltLeft{ 0b0001'0000 }; // represents bit 4
constexpr std::uint_fast8_t MASK_rotTiltRight{ 0b0010'0000 }; // represents bit 5
...
Color RayTracer::trace(Ray ray, int depth)
{
	...
	if (visualizeNormals == false)
	{
		...
		if (closestShape->m_mat.reflectiveIndex > 0.f)
		{
			...
			Color reflectionColor{ calulateReflection(ray.m_direction, intersectPoint, normalVec, closestShape, depth, fresnel) };
...
Color RayTracer::calulateReflection(const Vec3f& rayDirection, const Vec3f& intersectPoint, const Vec3f& normalVector, const Shape* closestShape, int depth, float fresnel)
{
	assert(closestShape != nullptr);
...
{
	float cosi =  ray.m_direction.dot(normal);
...
float RayTracer::Calculatefresnel(const Ray& ray, const Vec3f& normal, const Shape* closestShape)
{
	...
	if (sint >= 1.f) {
		...
		cosi = fabsf(cosi);
		float FresnelPer = ((etat * cosi) - (etai * cost)) / ((etat * cosi) + (etai * cost));
		float FresnelPar = ((etai * cosi) - (etat * cost)) / ((etai * cosi) + (etat * cost));
		return  (FresnelPer * FresnelPer + FresnelPar * FresnelPar) / 2.f;
	}
}
